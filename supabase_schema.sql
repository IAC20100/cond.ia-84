...
CREATE TABLE IF NOT EXISTS company_settings (
  ...
  bia_enabled BOOLEAN DEFAULT TRUE,
  energy_data JSONB DEFAULT '[]',
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- Inserir registro padrão de configurações se não existir
INSERT INTO company_settings (id, name, theme, menu_order) 
SELECT '00000000-0000-0000-0000-000000000000', 'CONDFY.IA', 'light', ARRAY['dashboard', 'accountability', 'consumption', 'clients', 'products', 'supplies', 'tickets', 'kanban', 'quotes', 'receipts', 'financial', 'calendar', 'settings']
...
CREATE TABLE IF NOT EXISTS contracts (
  ...
  payment_frequency TEXT NOT NULL CHECK (payment_frequency IN ('MENSAL', 'TRIMESTRAL', 'SEMESTRAL', 'ANUAL')),
...
-- 29. Tabela de Reformas (Renovations)
CREATE TABLE IF NOT EXISTS renovations (
  ...
  start_date TIMESTAMP WITH TIME ZONE NOT NULL,
  end_date TIMESTAMP WITH TIME ZONE NOT NULL,
  status TEXT NOT NULL CHECK (status IN ('PENDENTE', 'APROVADO', 'REALIZANDO', 'CONCLUIDO', 'REJEITADO')),
  art_file_url TEXT,
...
-- 30. Tabela de Mudanças (Moves)
CREATE TABLE IF NOT EXISTS moves (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  client_id UUID NOT NULL REFERENCES clients(id) ON DELETE CASCADE,
  date TIMESTAMP WITH TIME ZONE NOT NULL,
  type TEXT NOT NULL CHECK (type IN ('IN', 'OUT')),
  status TEXT NOT NULL CHECK (status IN ('PENDENTE', 'APROVADO', 'CONCLUIDO', 'CANCELADO')),
  notes TEXT,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- 31. Tabela de Regras de Cobrança (Billing Rules)
CREATE TABLE IF NOT EXISTS billing_rules (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  name TEXT NOT NULL,
  days_before_due INTEGER[] DEFAULT '{}',
  days_after_due INTEGER[] DEFAULT '{}',
...
-- 32. Tabela de Previsões Orçamentárias (Budget Forecasts)
CREATE TABLE IF NOT EXISTS budget_forecasts (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  month TEXT NOT NULL,
  monthly_projections JSONB NOT NULL, -- Array de objetos: [{month, value}]
  categories JSONB NOT NULL, -- Array de objetos: [{name, value}]
  insights TEXT[],
...
-- 36. Tabela de Controle de Chaves (Keys)
...
-- 37. Tabela de Estado IoT
CREATE TABLE IF NOT EXISTS iot_state (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  pumps JSONB NOT NULL DEFAULT '{"caixa": false, "jardim": false, "auto": true}',
  lights JSONB NOT NULL DEFAULT '{"cozinha": 0, "sala": 0, "jardim": 0, "todas": false}',
...
-- Inserir estado IoT padrão se não existir
INSERT INTO iot_state (id) 
SELECT '00000000-0000-0000-0000-000000000001'
WHERE NOT EXISTS (SELECT 1 FROM iot_state);

-- Habilitar Realtime para estas tabelas
ALTER PUBLICATION supabase_realtime ADD TABLE whatsapp_commands;
ALTER PUBLICATION supabase_realtime ADD TABLE notifications;
ALTER PUBLICATION supabase_realtime ADD TABLE critical_events;
ALTER PUBLICATION supabase_realtime ADD TABLE iot_state;
