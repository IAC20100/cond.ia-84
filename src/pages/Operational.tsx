...
export default function Operational() {
  ...
  const openModal = (item?: any) => {
    if (activeTab === 'STAFF') {
      ...
      if (item) {
        ...
        setKeyForm({ name: '', room: '', status: 'AVAILABLE', lastTakenBy: '', lastTakenAt: '', returnedAt: '' });
        setEditingId(null);
      }
  ...
  const filteredKeys = keys.filter(k => k.name.toLowerCase().includes(searchTerm.toLowerCase()));
        ...
        >
          ...
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              ...
              <p className="text-sm text-white/40">{key.room}</p>
            </div>
          </div>
          ...
          {key.status === 'BORROWED' && (
            <div className="mt-4 pt-4 border-t border-white/5">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium">{key.lastTakenBy}</span>
              </div>
              <p className="text-[10px] text-white/30 mt-1">Desde: {key.lastTakenAt ? safeFormatDate(key.lastTakenAt, { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' }) : '-'}</p>
            </div>
          )}
        </motion.div>
      ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="md:col-span-2">
                ...
                <input 
                  ...
                  type="text"
                  value={keyForm.room}
                  onChange={e => setKeyForm({...keyForm, room: e.target.value})}
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 outline-none focus:border-white/30 transition-all text-white"
                ...
                <div className="md:col-span-2 space-y-6">
                  <div>
                    ...
                    <input 
                      required
                      type="text"
                      value={keyForm.lastTakenBy}
                      onChange={e => setKeyForm({...keyForm, lastTakenBy: e.target.value})}
                      className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 outline-none focus:border-white/30 transition-all text-white"
                    />
                  </div>
                  <div>
                    ...
                    <input 
                      required
                      type="datetime-local"
                      value={keyForm.lastTakenAt}
                      onChange={e => setKeyForm({...keyForm, lastTakenAt: e.target.value})}
                      className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 outline-none focus:border-white/30 transition-all text-white"
                    />
                  </div>
                </div>
