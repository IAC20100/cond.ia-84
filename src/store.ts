    ...
    fetchInitialData: async () => {
      ...
      try {
        ...
        const results = await Promise.all([
          ...
          supabase.from('iot_state').select('*').single(),
        ...
        const [
          ...
          reservationsRes, staffRes, keysRes, iotStateRes, energyRecordsRes
        ] = results;
        ...
        // If IoT state doesn't exist, create a default row
        if (iotStateRes.error && iotStateRes.error.code === 'PGRST116') {
          const defaultIotState = {
            id: '00000000-0000-0000-0000-000000000001',
            pumps: { caixa: false, jardim: false, auto: true },
            lights: { cozinha: 0, sala: 0, jardim: 0, todas: false },
            alarm_active: false
          };
          const { data: newIotState, error: createError } = await supabase
            .from('iot_state')
            .insert([defaultIotState])
            .select()
            .single();
          
          if (!createError && newIotState) {
            iotStateRes.data = newIotState;
          }
        }
        ...
        if (billingRulesRes.data) {
          newState.billingRules = billingRulesRes.data.map(r => ({
            id: r.id,
            name: r.name,
            daysBeforeDue: r.days_before_due,
        ...
        if (keysRes.data) {
          newState.keys = keysRes.data.map(k => ({
            ...
            lastTakenBy: k.last_taken_by,
            lastTakenAt: k.last_taken_at,
        ...
        if (iotStateRes.data) {
          newState.iotState = {
            pumps: iotStateRes.data.pumps,
            lights: iotStateRes.data.lights,
            alarmActive: iotStateRes.data.alarm_active,
            waterLevel: iotStateRes.data.water_level,
            energyUsage: iotStateRes.data.energy_usage
          };
        }

        if (energyRecordsRes.data) {
          newState.energyData = energyRecordsRes.data.map(e => ({
            month: e.month,
            consumption: Number(e.consumption),
            solarGeneration: Number(e.solar_generation),
            sensorSavings: Number(e.sensor_savings),
            costWithoutTech: Number(e.cost_without_tech),
            actualCost: Number(e.actual_cost)
          }));
        }
        ...
        if (companySettingsRes.data) {
          ...
          newState.backgroundImage = companySettingsData.background_image;
          newState.theme = companySettingsData.theme as any;
          newState.whatsappEnabled = companySettingsData.whatsapp_enabled ?? true;
          newState.biaEnabled = companySettingsData.bia_enabled ?? true;
      ...
      toggleWhatsApp: async () => {
        const newValue = !get().whatsappEnabled;
        set({ whatsappEnabled: newValue });
        toast.success(`WhatsApp ${newValue ? 'ativado' : 'desativado'}`);
        
        const id = get().companySettingsId;
        if (id) {
          try {
            await supabase.from('company_settings').update({ whatsapp_enabled: newValue }).eq('id', id);
          } catch (e) { console.error(e); }
        }
      },
      toggleBia: async () => {
        const newValue = !get().biaEnabled;
        set({ biaEnabled: newValue });
        toast.success(`Bia ${newValue ? 'ativada' : 'desativada'}`);

        const id = get().companySettingsId;
        if (id) {
          try {
            await supabase.from('company_settings').update({ bia_enabled: newValue }).eq('id', id);
          } catch (e) { console.error(e); }
        }
      },
      ...
      addKey: async (key) => {
        ...
        try {
          await supabase.from('keys').insert([{
            ...
            room: key.room,
            status: key.status,
            last_taken_by: key.lastTakenBy,
            last_taken_at: key.lastTakenAt
          }]);
        } catch (e) { console.error(e); }
      },
      ...
      updateIotState: async (newState) => {
        set((state) => {
          ...
          // Persist to Supabase
          if (isSupabaseConfigured) {
            supabase.from('iot_state').upsert([{
              id: '00000000-0000-0000-0000-000000000001',
              pumps: finalIotState.pumps,
              lights: finalIotState.lights,
              water_level: finalIotState.waterLevel,
              energy_usage: finalIotState.energyUsage,
              last_update: new Date().toISOString()
            }]).then(({ error }) => {
              if (error) console.error('Erro Supabase updateIotState:', error);
            });
          }
